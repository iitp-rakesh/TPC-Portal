...
<body>
  ...
  $query = "SELECT * FROM company WHERE minimum_cpi <= $min_cpi AND jobId not IN ($applied_company)";
  ...
  if (isset($_POST['search_submit'])) {
    ...
    $conditions = array();
    $conditions[] = "minimum_cpi<='$min_cpi'";
    $conditions[] = "jobId not IN ($applied_company)";

    if (!empty($search_company_name)) {
      $conditions[] = "company_name LIKE '%$search_company_name%'";
    }

    if (!empty($search_min_cpi)) {
      $conditions[] = "minimum_cpi >= '$search_min_cpi'";
    }

    if (!empty($search_ctc)) {
      $conditions[] = "ctc >= '$search_ctc'";
    }

    if (!empty($search_role)) {
      $conditions[] = "roles LIKE '%$search_role%'";
    }
